...
{//此函数会让MOSFET的PWM输出“软件强制”为LOW
	EALLOW;
//sym: DB is bypassed for both output signals. In this mode, both EPWMxA & EPWMxB output
//		signals from the action-qualifier are passed directly to the PWM chopper module.
	EPwm1Regs.DBCTL.bit.OUT_MODE = DB_DISABLE;
	EPwm2Regs.DBCTL.bit.OUT_MODE = DB_DISABLE;
	EPwm3Regs.DBCTL.bit.OUT_MODE = DB_DISABLE;
	EPwm4Regs.DBCTL.bit.OUT_MODE = DB_DISABLE;
//sym: Forces a continuous LOW on ouput A & B.
//ref.[ePWM].P100.
...
{	//晶闸管旁路阻断
	SMGpioDataSet(8,GPIO_OUT_DOWN);		//GPIO_OUT_DOWN = 0
...
{	//晶闸管旁路导通
	MOSFETBlock( );
	SMGpioDataSet(8,GPIO_OUT_UP);		//GPIO_OUT_UP = 1
...
{	//有7个可用于显示的LED，应该就是面板上那8个LED中的7个，其中有一个可能没用到
...
//////////////////////////////////////////////////////////////////////////////////////////////
/*********SYM--外部信号输入作为PWM同步信号*****************************/
	SMGpioSet(32, GPIO_FUN_10, GPIO_R_UP, GPIO_DIR_IN, 3);
/***********************************************************/
}
